...
回到 queue 規則重建 `todo`，優先比較 `district_chengdu_market`、`district_chengdu_scholar` 與其他能提供 family variety 的非 `Wild` 候選，避免在 `wild_chengdu_outer` 後立刻再堆同質郊野節奏。

## Next Action

- 依 `world_map + progression + 題材分布表 + players/skills` 重建下一個 actionable area 候選
- 優先比較 `district_chengdu_market`、`district_chengdu_scholar` 與其他能提供 family variety 的非 `Wild` 候選
- 避免在 `wild_chengdu_outer` 後立刻再開另一段同質城外郊野節奏

## Next Prompt

`回到 queue 規則重建 todo，優先比較 district_chengdu_market、district_chengdu_scholar 與其他非 Wild 候選，選出下一個 actionable area。`
