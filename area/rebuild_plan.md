...
- `sec_rift_spirit_core_void`
  - plan: `plans/area/0027-sec-rift-spirit-core-void.md`
  - spec: `area/sec_rift_spirit_core_void/map.md`
  - reserved_room_block: `11201-11220`
  - theme: `仙俠`
  - subtheme: `裂魄空心 / 虛腹暗層`
  - ref_inputs_used:
  ...
  - theme_basis:
    ...
    - `ref/world-graph.md`
    - `ref/三國-MUD-題材分布表.md`
  - compliance_check: `compliant；依 sec_rift_spirit_core_hollow 的向下 world link 建立下一段 spec-first milestone，維持 ref/Readme.md 容許的 area rebuild 範圍`
...
目前 `sec_rift_spirit_core_void` 已完成 spec 建立與 validate-only 驗證，tracker 已切到 `implementation_in_progress`；本輪最合適的下一步是落地第一輪 runtime implementation，補齊最小可載入資料與 `11112 <-> 11201` 邊界出口。

下一輪建議動作：

1. 依 `area/sec_rift_spirit_core_void/map.md` 生成 `roo/11201-11212`
2. 補齊 `index / mob / obj / res / shp`
3. 更新 `area/directory.lst` 與 `area/sec_rift_spirit_core_hollow/roo/11112.roo` 的雙向邊界
...
- 目前已有 `in_progress` 項目 `sec_rift_spirit_core_void`，再次使用固定 prompt 時應優先續做此區，不得跳到其他候選
...
- 依新 room reservation policy，`sec_rift_below` 已使用 `9501-9520`，`sec_rift_core` 已使用 `9601-9620`，`sec_rift_deeps` 已完成 `9701-9712` 第一版落地，`sec_rift_prison_core` 已完成 `10001-10012` 第一版落地，`sec_rift_underseal_dais` 已完成 `10101-10112` 第一版落地，`sec_rift_true_core` 已完成 `10201-10212` 第一版落地，`sec_rift_core_below` 已完成 `10301-10312` 第一版落地，`sec_rift_core_below_deeps` 已完成 `10401-10412` 第一版落地，`sec_rift_core_vein_abyss` 已完成 `10501-10512` 第一版落地，`sec_rift_underseal_basin` 已完成 `10601-10612` 第一版落地，`sec_rift_sealed_core_basin` 已完成 `10701-10712` 第一版落地，`sec_rift_sealed_core_heart` 已完成 `10801-10812` 第一版落地，`sec_rift_core_spirit_nadir` 已完成 `10901-10912` 第一版落地，`sec_rift_core_spirit_abyss` 已完成 `11001-11012` 第一版落地，`sec_rift_spirit_core_hollow` 已完成 `11101-11112` 第一版落地，並保留各自首段 block
- 既有地下鏈 `9451/9461/9481/9491` 保留原狀，不在這一輪 retroactive 重編


建議可直接使用的 prompt：

續做 `sec_rift_spirit_core_void`：依 spec 落地第一輪 runtime implementation，補齊最小 area data 與 `11112 <-> 11201` 邊界。
