...
目前 `sec_rift_prison_core` 已完成第一輪 implementation；若要繼續使用固定 prompt，下一步需要先決定新的 candidate area，或規劃封印核心更下層的下一段單區 plan。

在沒有現成 `todo` 的前提下，下一輪建議可走兩條路：

1. 延續地下鏈，為 `sec_rift_prison_core` 之下的更深鎮封底座 / 裂界本核建立新的單區 plan。
...
- 依新 room reservation policy，`sec_rift_below` 已使用 `9501-9520`，`sec_rift_core` 已使用 `9601-9620`，`sec_rift_deeps` 已完成 `9701-9712` 第一版落地，`sec_rift_prison_core` 已完成 `10001-10012` 第一版落地
- 既有地下鏈 `9451/9461/9481/9491` 保留原狀，不在這一輪 retroactive 重編

建議可直接使用的 prompt：

`先盤點下一個可接續的 area candidate，或為 sec_rift_prison_core 之下的更深鎮封底座建立新的單區 plan。`
